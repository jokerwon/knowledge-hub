...
| Monorepo | web 与 api 共存于单一仓库；nest CLI 管理 api 与 libs/*，web 为游离 Next.js 应用（ADR-0020） | ADR-0004 |
...
| shared 契约库 | libs/shared（nest g library 生成）：前后端共享的 DTO 类型/枚举/常量，零框架依赖，类型即契约；api 经 @kh/shared 别名消费，web 引源码 | ADR-0020 |
