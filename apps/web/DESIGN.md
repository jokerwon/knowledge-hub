...
colors:
  ...
  light-surface-2: "#edeeee"
  light-surface-3: "#e6e7e9"
  light-surface-4: "#e0e1e4"
  light-hairline: "#e5e6ea"
  light-hairline-strong: "#d1d2d6"
  light-hairline-tertiary: "#c4c5c9"
  light-ink: "#18191a"
  light-ink-muted: "#3f434b"
  light-ink-subtle: "#6f747d"
  light-ink-tertiary: "#8a8f98"
  light-primary-hover: "#4a54cb"
  light-primary-focus: "#3d46a5"
  light-inverse-canvas: "#18191a"
  light-inverse-surface-1: "#26272a"
  light-inverse-surface-2: "#2e3034"
  light-inverse-ink: "#f7f8f8"
  light-edge-highlight: "rgba(255, 255, 255, 0.85)"
...
components:
  ...
  button-primary-hover-light:
    ...
    typography: "{typography.button}"
    rounded: "{rounded.md}"
  button-primary-pressed-light:
    backgroundColor: "{colors.light-primary-focus}"
    textColor: "{colors.on-primary}"
    typography: "{typography.button}"
    rounded: "{rounded.md}"
  button-inverse-light:
    backgroundColor: "{colors.light-inverse-canvas}"
    textColor: "{colors.light-inverse-ink}"
    typography: "{typography.button}"
    rounded: "{rounded.md}"
    padding: 8px 14px
---
...
Two canvases ship from one token set. The dark canvas is the default and the system's anchor; the light canvas is an explicit, persisted user choice toggled from the `top-nav` (`theme-toggle`) and applied before first paint. The lavender accent, radius, spacing, and typography scales are identical in both — only the surface/ink ladder, the accent's hover direction, and the inverse set change. See Colors → Light Theme.
...
- **Opt-in light mirror** — dark stays the anchor canvas; light re-derives the surface/ink ladder level-for-level (`{colors.light-*}`) and flips the inverse set.
...
The dark canvas remains the system's default and anchor. A light canvas ships as an explicit, persisted user choice — toggled from the `top-nav` via `theme-toggle`, stored per visitor, and applied before first paint so the canvas never flashes. `color-scheme` follows the active theme.

The light palette is a **mirror, not a second system**: the surface/ink/hairline ladder re-derives level-for-level, the lavender accent and all semantic colors stay identical, and radius/spacing/typography are untouched. The resolution rule is mechanical — in light mode, every `{colors.*}` reference resolves to its `{colors.light-*}` counterpart.

| Token | Dark | Light |
|---|---|---|
| `{colors.canvas}` → `{colors.light-canvas}` | #010102 | #ffffff |
| `{colors.surface-1}` → `{colors.light-surface-1}` | #0f1011 | #f5f6f6 |
| `{colors.surface-2}` → `{colors.light-surface-2}` | #141516 | #edeeee |
| `{colors.surface-3}` → `{colors.light-surface-3}` | #18191a | #e6e7e9 |
| `{colors.surface-4}` → `{colors.light-surface-4}` | #191a1b | #e0e1e4 |
| `{colors.hairline}` → `{colors.light-hairline}` | #23252a | #e5e6ea |
| `{colors.hairline-strong}` → `{colors.light-hairline-strong}` | #34343a | #d1d2d6 |
| `{colors.hairline-tertiary}` → `{colors.light-hairline-tertiary}` | #3e3e44 | #c4c5c9 |
| `{colors.ink}` → `{colors.light-ink}` | #f7f8f8 | #18191a |
| `{colors.ink-muted}` → `{colors.light-ink-muted}` | #d0d6e0 | #3f434b |
| `{colors.ink-subtle}` → `{colors.light-ink-subtle}` | #8a8f98 | #6f747d |
| `{colors.ink-tertiary}` → `{colors.light-ink-tertiary}` | #62666d | #8a8f98 |
| `{colors.edge-highlight}` → `{colors.light-edge-highlight}` | rgba(255,255,255,0.04) | rgba(255,255,255,0.85) |
| `{colors.primary-hover}` → `{colors.light-primary-hover}` | #828fff | #4a54cb |
| `{colors.primary-focus}` → `{colors.light-primary-focus}` | #5e69d1 | #3d46a5 |
| `{colors.inverse-canvas}` → `{colors.light-inverse-canvas}` | #ffffff | #18191a |
| `{colors.inverse-surface-1}` → `{colors.light-inverse-surface-1}` | #f5f6f6 | #26272a |
| `{colors.inverse-surface-2}` → `{colors.light-inverse-surface-2}` | #f6f7f7 | #2e3034 |
| `{colors.inverse-ink}` → `{colors.light-inverse-ink}` | #000000 | #f7f8f8 |

Shared in both canvases: `{colors.primary}`, `{colors.on-primary}`, `{colors.brand-secure}`, `{colors.semantic-success}`, `{colors.semantic-overlay}`.

Three behaviors diverge from the mechanical swap:

- **Accent hover/pressed darken instead of lift.** Dark hovers to `{colors.primary-hover}` #828fff; light hovers to `{colors.light-primary-hover}` #4a54cb and presses to `{colors.light-primary-focus}` #3d46a5 — the light states keep white labels above 5:1 contrast (light-UI convention: state changes darken).
- **The inverse set flips to the dark ladder.** The white pill CTA becomes charcoal: `{colors.light-inverse-canvas}` #18191a with `{colors.light-inverse-ink}` #f7f8f8 text (`button-inverse-light`).
- **Edge highlight strengthens.** The 4% white top sheen would vanish on near-white panels; light mode raises it to `{colors.light-edge-highlight}` (85% white).

> Provenance: the light ladder is derived by mirroring this spec — linear.app's marketing ships no light theme to extract from.
...
- **Subtle white edge highlight** ({colors.edge-highlight}) on the top edge of lifted panels — gives the dark surface a faint "pixel rendered" feel. Light mode raises the sheen to {colors.light-edge-highlight} so it survives the near-white panels.
...
- In light mode the state direction inverts — hover darkens to `{colors.light-primary-hover}` (#4a54cb) and pressed to `{colors.light-primary-focus}` (#3d46a5). Variant entries: `button-primary-hover-light`, `button-primary-pressed-light`.
- Marketing hero and banner CTAs render at a 44px-tall large size to hold the ≥40px touch-target rule; the compact 8px 14px spec covers dense UI chrome.
...
- In light mode the inverse flips to charcoal (`button-inverse-light`).
...
**`top-nav`** — Sticky dark bar with the Linear wordmark left, primary nav links centered, and a `button-secondary` ("Sign in") + `button-primary` ("Get started") pair right, plus a `theme-toggle` ghost icon button switching between the dark (default) and light canvases.
...
- Theme by swapping tokens, never hardcoding hex — in light mode every `{colors.*}` reference resolves to its `{colors.light-*}` mirror.

### Don't

- Don't flip the default canvas — dark is the anchor; light is an explicit, persisted user preference.
...
- **Display type**: `{typography.display-xl}` steps 80px → 56px at 768px → 36px at base; `{typography.display-lg}` 56px → 40px → 32px; `{typography.display-md}` 40px → 28px. Each step borrows the next-smaller cut's size and tracking.
...
- The light theme is a derived mirror spec (see Colors → Light Theme), not an extraction — linear.app's marketing ships no light theme to inspect.
